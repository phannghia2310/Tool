...
class Klink {
  ...
  async userInfo(userId, axiosInstance) {
  ...
  async dailyClaim(userId, axiosInstance) {
  ...
  async getAllTasks(userId, axiosInstance) {
  ...
  async excuteTask(userId, taskId, axiosInstance) {
  ...
  async manageTask(userId, axiosInstance) {
    const tasks = await this.getAllTasks(userId, axiosInstance);
    ...
    if (tasks) {
      for (const task of tasks) {
        if (task.completed) {
          ...
          const result = await this.excuteTask(userId, task._id, axiosInstance);
          if (result) {
  ...
  async main() {
    ...
    while (true) {
      for (let i = 0; i < data.length; i++) {
        ...
        const token = await this.getOrRefreshToken(i, initData, axiosInstance);
        if (!token) return null;
        this.headers["Authorization"] = `Bearer ${token}`;

        const user = await this.userInfo(userId, axiosInstance);
        if (user) {
          this.log(colors.green(`Balance: ${user.KlinkAirdropPoints}`));

          const dailyClaim = await this.dailyClaim(userId, axiosInstance);
