...
class XPlusApp {
  ...
  async countdown(t) {
    for (let i = t; i > 0; i--) {
      ...
      process.stdout.write(
        colors.yellow(`[*] Cần chờ ${hours}:${minutes}:${seconds}     \r`)
      );
  ...
  async checkin(token) {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        if (data.newCheckin) {
          const reward = data.configs.find(
            (config) => config.day === data.lastCheckinDay
          );
          if (reward) {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        if (now < finishTime) {
          this.log(
            `${colors.green(
              "Farming đang hoạt động. Thời gian hoàn thành:"
            )} ${colors.white(finishTime.toFormat("dd/MM/yyyy HH:mm:ss"))}`,
            "white"
          );
          return data;
        } else {
        ...
        this.log(
          `Không thể kiểm tra trạng thái farming. Mã trạng thái: ${res.status}`,
          "red"
        );
        return null;
      }
  ...
  async claimFarming(token) {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        this.log(
          `Không thể nhận phần thưởng farming. Mã trạng thái: ${res.status}`,
          "red"
        );
        return null;
      }
  ...
  async startFarming(token) {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        this.log(
          `${colors.green("Thời gian hoàn thành farm:")} ${colors.white(
            finishTime.toFormat("dd/MM/yyyy HH:mm:ss")
          )}`,
          "white"
        );
        return data;
        ...
        this.log(
          `Không thể bắt đầu farming. Mã trạng thái: ${res.status}`,
          "red"
        );
        return null;
      }
  ...
  async finishGame(token, points, hasBonus) {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        this.log(
          `${colors.yellow("Nhận được WTON:")} ${colors.white(points)}`,
          "white"
        );
        this.log(
          `${colors.yellow("Bonus:")} ${colors.white(hasBonus)}`,
          "white"
        );
        ...
        if (responseData.items && responseData.items.length > 0) {
          ...
          responseData.items.forEach((item) => {
            this.log(
              `${item.name} Tốc độ farm ${item.farmingPower} | ${item.tokenValue} WTON | ${item.value} TON`,
              "green"
            );
        ...
        this.log(
          `Không thể hoàn thành game. Mã trạng thái: ${res.status}`,
          "red"
        );
        return null;
      }
  ...
  async getTaskProgress(token) {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        this.log(
          `Không thể lấy tiến trình nhiệm vụ. Mã trạng thái: ${res.status}`,
          "red"
        );
      }
    } catch (error) {
  ...
  async processTasks(token) {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        this.log(
          `Không thể lấy danh sách nhiệm vụ. Mã trạng thái: ${res.status}`,
          "red"
        );
      }
    } catch (error) {
  ...
  async login(data, i) {
    ...
    try {
      ...
      if (userData) {
        console.log(
          `========== Tài khoản ${colors.yellow(
            userData.telegramId
          )} | ${colors.green(userData.firstName)} ==========`
        );
        this.log(
          `${colors.blue("WTON Balance:")} ${colors.white(
            userData.tokenBalance
          )}`,
          "white"
        );
        this.log(
          `${colors.blue("TON Balance:")} ${colors.white(
            userData.withdrawableBalance
          )}`,
          "white"
        );
        this.log(
          `${colors.blue("Ticket Count:")} ${colors.white(
        ...
        while (res.data.ticketCount > 0) {
          ...
          if (gameData) {
            ...
            const points = Math.floor(Math.random() * (1000 - 900 + 1)) + 900;
  ...
  async getUserData(token) {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        this.log(
          `Không thể lấy dữ liệu người dùng. Mã trạng thái: ${res.status}`,
          "red"
        );
        return null;
      }
    ...
    while (true) {
      ...
      if (this.firstFinishTime) {
        ...
        const countdownTime = Math.max(
          0,
          Math.floor(this.firstFinishTime.diff(now).as("seconds"))
        );
