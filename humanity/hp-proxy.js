...
class HumanityClient {
  ...
  async makeRequest(url, method = "get", data = null) {
    const config = {
      ...
      httpsAgent: this.currentProxy
        ? new HttpsProxyAgent(this.currentProxy)
        : undefined,
      timeout: 30000,
    };
  ...
  async main() {
    const walletFile = path.join(__dirname, "wallet2.txt");
    const privateFile = path.join(__dirname, "private2.txt");
