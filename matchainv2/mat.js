...
class Matchain {
  ...
  async completeQuiz() {
    try {
      const quizUrl =
        "https://tgapp-api.matchain.io/api/tgapp/v1/daily/quiz/progress";
      ...
      const submitUrl =
        "https://tgapp-api.matchain.io/api/tgapp/v1/daily/quiz/submit";
  ...
  async login(data) {
    ...
    res = await this.http(
      balanceUrl,
      this.headers,
      JSON.stringify({ uid: this.userid })
    );
    if (res.status !== 200) {
      ...
      if (taskStatus.dailyNeedsPurchase) {
        try {
          ...
          if (boosterResult.code === 400) {
            this.log(
              "Bạn đã thực hiện mua booster trước đó, thử lại sau!",
              "warning"
            );
    ...
    while (true) {
      ...
      if (next_claim > Date.now()) {
        const format_next_claim = DateTime.fromMillis(next_claim).toFormat(
          "yyyy-MM-dd HH:mm:ss"
        );
    ...
    while (gameCount > 0) {
      ...
      const point = Math.floor(Math.random() * (165 - 160 + 1)) + 160;
  ...
  async getTaskList(uid) {
    ...
    if (res.status !== 200) {
      this.log(
        `Lỗi khi lấy danh sách nhiệm vụ! Status: ${res.status}`,
        "error"
      );
      return false;
    }
    ...
    const extraTasks = Array.isArray(data["Extra Tasks"])
      ? data["Extra Tasks"]
      : [];
    const allTasks = [...data.Tasks, ...extraTasks];
    const filteredTasks = allTasks.filter(
      (task) => task.complete === false && task.name !== "join_match_group"
    );
  ...
  async completeTask(uid, taskType) {
    ...
    if (res.status !== 200) {
      this.log(
        `Lỗi khi hoàn thành nhiệm vụ ${taskType}! Status: ${res.status}`,
        "error"
      );
  ...
  async claimReward(uid, taskType) {
    ...
    if (res.status !== 200) {
      this.log(
        `Lỗi khi nhận phần thưởng nhiệm vụ ${taskType}! Status: ${res.status}`,
        "error"
      );
      this.log(`Response: ${JSON.stringify(res.data)}`, "error");
      return false;
    ...
    if (res.data.code === 200 && res.data.data === "success") {
      this.log(
        `${"Làm nhiệm vụ".yellow} ${taskType.white} ... ${
          "Trạng thái:".white
        } ${"Hoàn thành".green}`
      );
    } else {
  ...
  async checkDailyTaskStatus() {
    ...
    try {
      ...
      return {
        dailyNeedsPurchase:
          dailyTask && dailyTask.current_count < dailyTask.task_count,
        gameNeedsPurchase:
          gameTask && gameTask.current_count < gameTask.task_count,
      };
    } catch (error) {
      this.log(
        `Lỗi khi kiểm tra trạng thái nhiệm vụ: ${error.message}`,
        "error"
      );
      return null;
    }
