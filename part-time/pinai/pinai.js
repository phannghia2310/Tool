...
class Pinai {
    ...
    async getHomeData(token, hoinangcap) {
            ...
            if (response.status === 200) {
                ...
                for (const task of tasks) {
                    if (task.task_id === 1001 && task.checkin_detail.is_today_checkin === 0) {
                        await this.completeTask(token, task.task_id, "Điểm danh hàng ngày thành công");
                    } else if (task.is_complete === false) {
                        await this.completeTask(token, task.task_id, `Làm nhiệm vụ ${task.task_name} thành công | Phần thưởng: ${task.reward_points}`);
                    }
                }
            } else {
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                if (!tokenData[userId] || this.isExpired(tokenData[userId].access_token)) {
                    ...
                    if (newToken) {
                        ...
                        await this.getHomeData(newToken, hoinangcap);
                        await this.getTasks(newToken);
                    }
                } else {
                    this.log(`Token hợp lệ cho tài khoản ${userId}. Không cần đăng nhập lại.`, 'success');
                    await this.getHomeData(tokenData[userId].access_token, hoinangcap);
