...
class Bums {
  ...
  async finishTask(token, taskId, taskInfo, proxyUrl) {
    ...
    const episodeCodes = {
        0: "42858", 1: "95065", 2: "88125", 3: "51264", 4: "13527", 5: "33270", 6: "57492", 7: "63990", 8: "19988", 9: "26483",
        10: "36624", 11: "30436", 12: "71500", 13: "48516", 14: "92317", 15: "68948", 16: "98109", 17: "35264", 18: "86100", 19: "86100",
        20: "83273", 21: "74737", 22: "18948", 23: "16086", 24: "13458", 25: "13458", 26: "91467", 27: "71728", 28: "97028", 29: "97028",
        30: "89349", 31: "31114", 32: "31114", 33: "37422", 34: "52860", 35: "10300", 36: "35583", 37: "35194", 38: "26488", 39: "85133",
        40: "13116", 41: "28932", 42: "50662", 43: "83921", 44: "35176", 45: "24345", 46: "95662", 47: "43700", 48: "36632", 49: "74504",
        50: "74507", 51: "46056", 52: "48627", 53: "39617", 54: "15876", 55: "60465", 56: "68600", 57: "27171", 58: "61306", 59: "55380",
        60: "71506", 61: "46336", 62: "74624", 63: "20437", 64: "15402", 65: "49946", 66: "31223", 67: "29364", 68: "19185", 69: "38190",
        70: "46454", 71: "49697", 72: "35532", 73: "23488", 74: "38242", 75: "47334", 76: "22332", 77: "44088", 78: "49993", 79: "30571",
        80: "33202", 81: "28587", 82: "10317", 83: "44543", 84: "13028", 85: "27933",
    };
    ...
    if (
      ...
      if (episodeNum !== null && episodeCodes[episodeNum]) {
        ...
        this.log(
          `Đang gửi mã cho Episode ${episodeNum}: ${episodeCodes[episodeNum]}`,
          "info"
        );
      }
    }
  ...
  async processTasks(token, proxyUrl) {
    ...
    for (const task of taskList.tasks) {
      ...
      if (result.success) {
        this.log(
          `Làm nhiệm vụ ${task.name} thành công | Phần thưởng: ${task.rewardParty}`,
          "success"
        );
      } else {
        this.log(
          `Không thể hoàn thành nhiệm vụ ${task.name}: ${result.error}`,
          "error"
        );
      }
  ...
  async processEnergyCollection(token, energy, initialCollectSeqNo, proxyUrl) {
    ...
    for (let i = 0; i < energyDistributions.length; i++) {
      ...
      if (result.success) {
        ...
        this.log(
          `Thành công! Đã thu thập: ${totalCollected}/${energy}`,
          "success"
        );
      } else {
  ...
  async processMineUpgrades(token, currentCoin, proxyUrl) {
    ...
    let availableMines = mineList.mines
      .filter(
        (mine) =>
          mine.status === 1 &&
          parseInt(mine.nextLevelCost) <=
            Math.min(currentCoin, config.maxUpgradeCost)
      )
      .sort(
        (a, b) => parseInt(b.nextPerHourReward) - parseInt(a.nextPerHourReward)
      );
    ...
    for (const mine of availableMines) {
      ...
      this.log(
        `Đang nâng cấp thẻ ID ${mine.mineId} | Cost: ${cost} | Reward/h: ${mine.nextPerHourReward}`,
        "info"
      );
      ...
      if (result.success) {
        remainingCoin -= cost;
        this.log(
          `Nâng cấp thẻ ID ${mine.mineId} thành công | Remaining coin: ${remainingCoin}`,
          "success"
        );
      } else {
        this.log(
          `Không thể nâng cấp thẻ ID ${mine.mineId}: ${result.error}`,
          "error"
        );
      }
  ...
  async processSignIn(token, proxyUrl) {
    ...
    if (result.success) {
      this.log(
        `Điểm danh ngày ${availableDay.days} thành công | Phần thưởng: ${availableDay.normal}`,
        "success"
      );
    } else {
  ...
  async joinGang(token, gangName = "dancayairdrop", proxyUrl) {
  ...
  async processGangJoin(token, proxyUrl) {
    ...
    if (!gangList.myGang.gangId) {
      this.log(
        "Bạn chưa tham gia gang nào, đang thử gia nhập Gang Dân Cày Airdrop...",
        "info"
      );
      ...
      this.log(
        `Bạn đã là thành viên của gang ${gangList.myGang.name}`,
        "custom"
      );
    }
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        try {
          ...
          if (!loginResult.success) {
            this.log(
              `Đăng nhập không thành công: ${loginResult.error}`,
              "error"
            );
            continue;
          }
          ...
          if (gameInfo.success) {
            ...
            if (parseInt(gameInfo.energySurplus) > 0) {
              ...
              const collectSeqNo =
                gameInfo.data.tapInfo.collectInfo.collectSeqNo;
              await this.processEnergyCollection(
                token,
                gameInfo.energySurplus,
                collectSeqNo,
                currentProxy
              );
            } else {
            ...
            this.log(
              `Không thể lấy thông tin game: ${gameInfo.error}`,
              "error"
            );
          }
          ...
          if (hoinangcap) {
            await this.processMineUpgrades(
              token,
              parseInt(gameInfo.coin),
              currentProxy
            );
          }
