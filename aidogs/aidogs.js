...
class warlockAPIClient {
  ...
  async generateToken(initData) {
    ...
    try {
      ...
      this.log(`Lỗi khi lấy token : ${error.message}`, "error");
      return null;
    }
  }
  ...
  async fetchUserData(initData) {
    ...
    try {
      ...
      if (response.status === 200) {
        this.log(
          `Lấy dữ liệu người dùng ${userData.username} thành công!`,
          "success"
        );
        return {
          success: true,
  ...
  async claimEarlyAdopterBonus(userData) {
    ...
    try {
      ...
      if (
        response.status === 200 &&
        response.data.message === "Points updated successfully"
      ) {
        this.log(
          `Bonus cập nhật thành công cho người dùng ${userData.username}`,
          "success"
        );
        console.log(
          `PointsNo: ${response.data.userData.pointsNo}, UserLevel: ${response.data.userData.userLevel}`
        );
        return true;
      }
    } catch (error) {
  ...
  async checkDailyRewardStatus(userData) {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (reward.__v === 0) {
          ...
          const lastClaimDate = new Date(
            reward.dailyClaims[reward.dailyClaims.length - 1].date
          );
          const currentDate = new Date();

          if (currentDate.getDate() !== lastClaimDate.getDate()) {
            this.log(
              "Ngày hôm nay đã qua 1 ngày, thực hiện điểm danh lại...",
              "custom"
            );
            await this.claimDailyReward(userData);
          } else {
      ...
      this.log(
        `Lỗi khi kiểm tra trạng thái điểm danh: ${error.message}`,
        "error"
      );
    }
  }
  ...
  async claimDailyReward(userData) {
    ...
    try {
      ...
      if (
        ...
        this.log(
          `Checkin hàng ngày thành công | Nhận ${response.data.totalPoints} points`,
          "success"
        );
        return true;
      }
  ...
  async performSocialTasks(userData) {
    ...
    const retryApiCall = async (apiCall) => {
      for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        try {
          ...
          if (
            error.response &&
            (error.response.status === 504 || error.response.status === 502)
          ) {
            this.log(
              `Attempt ${attempt}: 504 Gateway Timeout. Retrying in 3 seconds...`,
              "warning"
            );
            ...
            if (attempt === MAX_RETRIES) {
              this.log(
                "Max retries reached. Unable to complete the API call.",
                "error"
              );
              throw error;
            }
    ...
    try {
      ...
      const fourResponse = await retryApiCall(
        async () =>
          await axios.post(
            updateTasksFourUrl,
            { user: userData },
      ...
      const socialRewardDeets =
        oneResponse.data.userData.socialRewardDeets ||
      ...
      for (const task of activeSocialTasks) {
        ...
        this.log(
          `Task "${task.taskText}" completed successfully | Reward: ${task.taskPoints} points`,
          "success"
        );
      }
  ...
  async main() {
    ...
    while (true) {
      for (let i = 0; i < data.length; i++) {
        ...
        const userDataa = JSON.parse(
          decodeURIComponent(initData.split("user=")[1].split("&")[0])
        );

        console.log(
          `========== Tài khoản ${i + 1} | ${
            userDataa.username.green
          } ==========`
        );

        const token = await this.generateToken(initData);
        ...
        const pointsNo =
          userData.pointsNo !== undefined ? userData.pointsNo : "Chưa có điểm";
        const userLevel =
          userData.userLevel !== undefined
            ? userData.userLevel
            : "Chưa có cấp độ";
        if (userDataResult.success) {
          if (!userDataResult.data.userData.earlyAdopterBonusClaimed) {
            await this.claimEarlyAdopterBonus(
              userDataResult.data.userData.user
            );
          } else {
          ...
          this.log(
            `Không thể lấy dữ liệu người dùng: ${userDataResult.error}`,
            "error"
          );
        }
