...
class Tomarket {
  ...
  async checkRank() {
    const rankDataUrl =
      "https://api-web.tomarket.ai/tomarket-game/v1/rank/data";
    const evaluateUrl =
      "https://api-web.tomarket.ai/tomarket-game/v1/rank/evaluate";
    const createRankUrl =
      "https://api-web.tomarket.ai/tomarket-game/v1/rank/create";
    ...
    try {
      ...
      if (createRankRes.status === 200 && createRankRes.data.status === 0) {
        ...
        this.log(
          colors.green(
            `Kiểm tra rank thành công, rank hiện tại: ${currentRank.name}`
          )
        );
      }
    } catch (error) {
  ...
  async checkTaskStatus(taskId, initData) {
    ...
    try {
      ...
      if (res.status === 200 && res.data.status === 0) {
        ...
        this.log(
          colors.yellow(
            `Không thể kiểm tra trạng thái nhiệm vụ ${taskId}. Response:`,
  ...
  async processTasks(tasks, type, initData, maxRetries = 5) {
    const tasksToProcess = tasks.filter((task) =>
      type === "default" ? task.status === 0 : true
    );
    ...
    for (const task of tasksToProcess) {
      if (type !== "default") {
        ...
        if (startResult === true) {
          this.log(
            colors.green(
              `Bắt đầu nhiệm vụ ${task.taskId}: ${task.title} thành công`
            )
          );
          allTasksCompleted = false;
        } else if (startResult === "completed") {
          this.log(
            colors.blue(`Nhiệm vụ ${task.taskId}: ${task.title} đã được làm`)
          );
        } else {
          this.log(
            colors.yellow(
              `Không thể bắt đầu nhiệm vụ ${task.taskId}: ${task.title} sau ${maxRetries} lần thử`
            )
          );
          allTasksCompleted = false;
        }
    ...
    if (type !== "default") {
      if (!allTasksCompleted) {
        this.log(
          colors.blue("Chờ 31 giây trước khi kiểm tra trạng thái nhiệm vụ...")
        );
      ...
      for (const task of tasksToProcess) {
        ...
        if (status === 2) {
          ...
          if (claimed) {
            this.log(
              colors.green(
                `Nhận thưởng nhiệm vụ ${task.taskId}: ${task.title} thành công | phần thưởng: ${task.score}`
              )
            );
          } else {
            this.log(
              colors.yellow(
                `Không thể nhận thưởng nhiệm vụ ${task.taskId}: ${task.title}`
              )
            );
          }
        } else if (status === 3) {
          this.log(
            colors.blue(`Nhiệm vụ ${task.taskId}: ${task.title} đã hoàn thành`)
          );
        } else {
          this.log(
            colors.yellow(
              `Nhiệm vụ ${task.taskId}: ${task.title} chưa hoàn thành (status: ${status})`
            )
          );
        }
        await this.countdown(3);
