...
class SidekickAPIClient {
    ...
    loadProxies() {
        try {
            ...
            this.proxyList = fs.readFileSync(proxyFile, 'utf8')
                ...
                .split('\n')
                .filter(Boolean);
            this.log(`Loaded ${this.proxyList.length} proxies`, 'success');
        } catch (error) {
            this.log('Error loading proxy list: ' + error.message, 'error');
            this.proxyList = [];
        }
    }
    ...
    async connectWebSocket(token, init, proxy = null) {
        ...
        while (true) {
            for (let i = 0; i < data.length; i++) {
                ...
                if (loginResult.success) {
                    ...
                    try {
                        await this.connectWebSocket(token, init, proxy);
                    } catch (error) {
                        this.log(`Lỗi rồi: ${error.message}`, 'error');
                    }
                } else {
