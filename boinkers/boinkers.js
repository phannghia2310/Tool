...
class Boink {
  ...
  async loginByTelegram(initDataString) {
    const url =
      "https://boink.boinkers.co/public/users/loginByTelegram?p=android";
    ...
    try {
      ...
      if (response.status === 200 && response.data) {
        const { newSoftCurrencyAmount, newSlotMachineEnergy, rank } =
          response.data;
        this.log(
          `Nâng cấp thành công, Coin: ${newSoftCurrencyAmount} | Spin: ${newSlotMachineEnergy} | Rank: ${rank}`,
          "success"
        );
        return { success: true };
      } else {
        this.log(
          `Nâng cấp thất bại! Mã trạng thái: ${response.status}`,
          "error"
        );
    ...
    while (remainingSpins > 0) {
      ...
      try {
        ...
        if (response.status === 200) {
          const result = response.data;
          this.log(
            `Spin thành công (${result.outcome}): Coin: ${
              result.newSoftCurrencyAmount.toString().white
            }${` | Shit: `.magenta}${
              result.newCryptoCurrencyAmount.toFixed(2).white
            }`.magenta,
            "custom"
          );
  ...
  async performRewardedActions(token) {
    ...
    try {
      ...
      if (response.status !== 200) {
        this.log(
          `Không thể lấy danh sách nhiệm vụ. Mã trạng thái: ${response.status}`,
          "error"
        );
        return;
      }
      ...
      for (const action of rewardedActions) {
        ...
        if (!canPerformTask) {
          if (waitTime) {
            ...
            this.log(
              `Cần chờ ${waitMinutes} phút để tiếp tục làm nhiệm vụ ${nameId}`,
              "info"
            );
          } else {
        ...
        if (
          ...
          try {
            ...
            this.log(
              `Làm nhiệm vụ ${nameId.yellow}. trạng thái: ${`pending`.yellow}`
            );
          } catch (clickError) {
            this.log(
              `Lỗi khi làm nhiệm vụ ${nameId}: ${clickError.message}`,
              "error"
            );
            if (clickError.response) {
              this.log(
                `Chi tiết lỗi: ${JSON.stringify(clickError.response.data)}`,
                "error"
              );
            }
            continue;
          ...
          try {
            ...
            if (claimResponse.status === 200) {
              ...
              this.log(
                `Hoàn thành nhiệm vụ ${nameId} thành công | Phần thưởng: ${reward}`,
                "success"
              );
            } else {
              this.log(
                `Không thể nhận thưởng cho ${nameId}. Mã trạng thái: ${claimResponse.status}`,
                "error"
              );
            }
          } catch (claimError) {
            this.log(
              `Lỗi khi nhận thưởng cho ${nameId}: thời gian chờ vẫn còn!`,
              "error"
            );
          }
        }
      ...
      if (error.response) {
        this.log(
          `Chi tiết lỗi: ${JSON.stringify(error.response.data)}`,
          "error"
        );
      }
    }
  ...
  async handleAdTask(token, nameId, providerId) {
    ...
    try {
      ...
      const claimUrl = `https://boink.astronomica.io/api/rewardedActions/claimRewardedAction/${nameId}?p=android`;
      this.log(
        `Gửi yêu cầu nhận thưởng cho nhiệm vụ quảng cáo ${nameId}...`,
        "info"
      );
      const claimResponse = await axios.post(claimUrl, {}, { headers });
      ...
      if (claimResponse.status === 200) {
        ...
        const reward = result.prizeGotten;
        this.log(
          `Hoàn thành nhiệm vụ quảng cáo ${nameId} thành công | Phần thưởng: ${reward}`,
          "success"
        );
      } else {
        this.log(
          `Không thể nhận thưởng cho nhiệm vụ quảng cáo ${nameId}. Mã trạng thái: ${claimResponse.status}`,
          "error"
        );
      }
    } catch (error) {
      this.log(
        `Lỗi khi xử lý nhiệm vụ quảng cáo ${nameId}: thời gian chờ vẫn còn!`,
        "error"
      );
    }
  }
  ...
  async main() {
    ...
    while (true) {
      for (let i = 0; i < data.length; i++) {
        ...
        if (!token) {
          this.log(
            `Không tìm thấy token cho tài khoản ${userId}, đăng nhập...`,
            "warning"
          );
        ...
        try {
          ...
          if (userInfoResult.success) {
            ...
            this.log(
              `Level: ${userInfo.boinkers.currentBoinkerProgression.level}`,
              "info"
            );
            ...
            this.log(
              `Spin: ${userInfo.gamesEnergy.slotMachine.energy}`,
              "info"
            );
            ...
            if (
              ...
              this.log(
                `Thời gian mua boosts tiếp theo: ${nextBoosterTime.toLocaleString(
                  DateTime.DATETIME_MED
                )}`,
                "info"
              );
