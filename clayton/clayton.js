...
class Clayton {
  ...
  async fetchApiBaseId() {
    try {
      const response = await axios.get(
        "https://tonclayton.fun/assets/index-Cw_PkCi2.js"
      );
      const jsContent = response.data;

      // Tìm API base ID từ nội dung file JS
      const match = jsContent.match(/Yge\s*=\s*"([^"]+)"/);
      if (match && match[1]) {
  ...
  async login(initData) {
    const url =
      `https://tonclayton.fun/api/${this.apiBaseId}/user/authorization`;
  ...
  async dailyClaim(initData) {
    const url =
      `https://tonclayton.fun/api/${this.apiBaseId}/user/daily-claim`;
    const headers = { ...this.headers, "Init-Data": initData };
    try {
      const response = await axios.post(url, {}, { headers });
      if (response.status === 200) {
  ...
  async startFarm(initData) {
    const url =
      `https://tonclayton.fun/api/${this.apiBaseId}/game/start`;
    const headers = {
      ...this.headers,
  ...
  async claimFarm(initData) {
    const url =
      `https://tonclayton.fun/api/${this.apiBaseId}/user/claim`;
    const headers = { ...this.headers, "Init-Data": initData };
    try {
      const response = await axios.post(url, {}, { headers });
      if (response.status === 200) {
  ...
  async getPartnerTasks(initData) {
    const url =
      `https://tonclayton.fun/api/${this.apiBaseId}/tasks/partner-tasks`;
    const headers = { ...this.headers, "Init-Data": initData };
    try {
      const response = await axios.post(url, {}, { headers });
      this.log(response);
  ...
  async completePartnerTask(initData, taskId) {
    const url = `https://tonclayton.fun/api/${this.apiBaseId}/tasks/complete/${taskId}`;
    const headers = { ...this.headers, "Init-Data": initData };
    try {
      const response = await axios.post(url, {}, { headers });
      if (response.status === 200) {
  ...
  async rewardPartnerTask(initData, taskId) {
    const url = `https://tonclayton.fun/api/${this.apiBaseId}/tasks/claim/${taskId}`;
    const headers = { ...this.headers, "Init-Data": initData };
    try {
      const response = await axios.post(url, {}, { headers });
      if (response.status === 200) {
  ...
  async handlePartnerTasks(initData) {
    ...
    if (tasksResult.success) {
      ...
      for (const task of uncompletedTasks) {
        ...
        if (completeResult.success) {
          ...
          if (rewardResult.success) {
            ...
            this.log(
              `Không thể nhận phần thưởng cho nhiệm vụ ${task.task_name}: ${
                rewardResult.error || "Lỗi không xác định"
              }`,
              "error"
            );
          }
          ...
          this.log(
            `Không thể hoàn thành nhiệm vụ ${task.task_name}: ${
              completeResult.error || "Lỗi không xác định"
            }`,
            "error"
          );
        }
      ...
      this.log(
        `Không thể lấy danh sách nhiệm vụ đối tác: ${
          tasksResult.error || "Lỗi không xác định"
        }`,
        "error"
      );
    }
  ...
  async handleTwitterTask(initData) {
    const checkUrl =
      "https://tonclayton.fun/api/${this.apiBaseId}/tasks/default-tasks";
    const claimUrl =
      "https://tonclayton.fun/api/${this.apiBaseId}/tasks/default-tasks/claim";
    const headers = { ...this.headers, "Init-Data": initData };
  ...
  async handleBotTask(initData) {
    const checkUrl =
      `https://tonclayton.fun/api/${this.apiBaseId}/user/task-bot`;
    const claimUrl =
      `https://tonclayton.fun/api/${this.apiBaseId}/user/task-bot-claim`;
    const headers = { ...this.headers, "Init-Data": initData };
    ...
    try {
      ...
      if (
        ...
        if (claimResponse.data.claimed) {
          this.log(
            `Làm nhiệm vụ use bot thành công. Nhận được ${claimResponse.data.claimed} CL`,
            "success"
          );
        } else {
  ...
  async handleDailyTasks(initData) {
    const dailyTasksUrl =
      `https://tonclayton.fun/api/${this.apiBaseId}/tasks/daily-tasks`;
    const headers = { ...this.headers, "Init-Data": initData };
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        for (const task of uncompletedTasks) {
          const completeUrl = `https://tonclayton.fun/api/${this.apiBaseId}/tasks/daily-task/${task.id}/complete`;
          const claimUrl = `https://tonclayton.fun/api/${this.apiBaseId}/tasks/daily-task/${task.id}/claim`;
          ...
          try {
            ...
            if (claimResponse.data.message === "Reward claimed successfully") {
              this.log(
                `Làm nhiệm vụ ${task.task_type} thành công | Nhận ${claimResponse.data.reward} CL`,
                "success"
              );
            } else {
              this.log(
                `Không thể nhận phần thưởng cho nhiệm vụ ${task.task_type}`,
                "error"
              );
            }
          } catch (error) {
  ...
  async playGame(initData) {
    const headers = { ...this.headers, "Init-Data": initData };
    const baseUrl =
      `https://tonclayton.fun/api/${this.apiBaseId}`;
    ...
    while (true) {
      ...
      while (startAttempts < 1 && !gameStarted) {
        try {
          ...
          if (startGameResponse.data.message === "Game started successfully") {
            sessoin_id = startGameResponse.data.sessoin_id;
      ...
      if (!gameStarted) {
        this.log(
          "Không thể bắt đầu trò chơi sau 3 lần thử. Dừng chơi game.",
          "error"
        );
        return;
      }
      ...
      for (const milestone of allMilestones) {
        ...
        try {
          const saveGameResponse = await axios.post(
            ...
            { maxTile: milestone, sessoin_id: sessoin_id },
            { headers }
          );
      ...
      try {
        ...
        this.log(
          `Trò chơi đã kết thúc thành công. Nhận ${reward.earn} CL và ${reward.xp_earned} XP`,
          "custom"
        );
      } catch (error) {
    ...
    while (true) {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        try {
          ...
          if (!apiBaseIdSuccess) {
            this.log(
              "Failed to initialize API Base ID. Skipping account.",
              "error"
            );
            return;
          }
        ...
        if (loginResult.success) {
          ...
          if (loginResult.data.dailyReward.can_claim_today) {
            ...
            if (
              ...
              this.log(
                `Không thể nhận phần thưởng hàng ngày: ${
                  claimResult.error || "Lỗi không xác định"
                }`,
                "error"
              );
            }
          ...
          if (!userInfo.active_farm) {
            ...
            if (startResult.success) {
              ...
              this.log(
                `Farm bắt đầu. Thời gian hoàn thành: ${finishTime.toFormat(
                  "dd/MM/yyyy HH:mm:ss"
                )}`,
                "success"
              );
              ...
              this.log(
                `Không thể bắt đầu farm: ${
                  startResult.error || "Lỗi không xác định"
                }`,
                "error"
              );
            }
            ...
            if (!userInfo.can_claim) {
              ...
              this.log(
                `Farm đang hoạt động. Thời gian hoàn thành: ${finishTime.toFormat(
                  "dd/MM/yyyy HH:mm:ss"
                )}`,
                "info"
              );
              ...
              if (claimResult.success) {
                this.log(
                  `Claim thành công. Nhận ${claimResult.data.claim} CL và ${claimResult.data.xp_earned} XP | Balance: ${claimResult.data.tokens}`,
                  "success"
                );
                ...
                if (startResult.success) {
                  ...
                  this.log(
                    `Farm mới bắt đầu. Thời gian hoàn thành: ${finishTime.toFormat(
                      "dd/MM/yyyy HH:mm:ss"
                    )}`,
                    "success"
                  );
                ...
                this.log(
                  `Không thể nhận phần thưởng farm: ${
                    claimResult.error || "Lỗi không xác định"
                  }`,
                  "error"
                );
      ...
      if (this.firstAccountFarmEndTime) {
        ...
        if (waitTime > 0) {
          this.log(
            `Chờ đến khi farm của tài khoản đầu tiên hoàn thành...`,
            "info"
          );
        ...
        this.log(
          `Không có thông tin về thời gian hoàn thành farm của tài khoản đầu tiên. Chờ 6 giờ...`,
          "warning"
        );
