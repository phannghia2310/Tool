...
class TonCircle {
  ...
  async processTasksOfType(authorization, type, proxy) {
    ...
    if (tasksResult.success) {
      const incompleteTasks = tasksResult.data.tasks.filter(
        (task) => !task.completed
      );
      for (const task of incompleteTasks) {
        const startResult = await this.startTask(
          authorization,
          task.id,
          type,
          proxy
        ...
        if (startResult.success) {
          const finalizeResult = await this.finalizeTask(
            authorization,
            task.id,
            type,
            proxy
          ...
          if (finalizeResult.success) {
            this.log(
              `Làm nhiệm vụ ${task.data.title} (${type}) thành công | phần thưởng : ${task.reward}`,
              "success"
            );
          } else {
            this.log(
              `Làm nhiệm vụ ${task.data.title} (${type}) không thành công | Cần tự làm`,
              "error"
            );
          }
        } else {
          this.log(
            `Không thể bắt đầu nhiệm vụ ${task.data.title} (${type}): ${startResult.error}`,
            "error"
          );
        }
        await new Promise((resolve) => setTimeout(resolve, 1000));
      ...
      this.log(
        `Không thể lấy danh sách nhiệm vụ ${type}: ${tasksResult.error}`,
        "error"
      );
    }
  }

  async spin(authorization, bet, proxy) {
    const url = `https://api.toncircle.org/user/games/roulette/spin?c=${Date.now()}`;
    const headers = { Authorization: authorization };
    const payload = { amount: bet };
    return this.makeRequest(url, "post", payload, headers, proxy);
  }
  ...
  async watchAd(adData, proxy) {
    try {
      ...
      const renderTracking = adData.banner.trackings.find(
        (t) => t.name === "render"
      );
      if (renderTracking) {
      ...
      const showTracking = adData.banner.trackings.find(
        (t) => t.name === "show"
      );
      if (showTracking) {
      ...
      const rewardTracking = adData.banner.trackings.find(
        (t) => t.name === "reward"
      );
      if (rewardTracking) {
  ...
  async processAdWatching(authorization, tg_id, proxy) {
    ...
    if (profileResult.success) {
      const adWatchTime = DateTime.fromISO(profileResult.data.adWatchTime, {
        zone: "utc",
      }).setZone("local");
      ...
      if (now < adWatchTime) {
        ...
        this.log(
          `Thời gian xem quảng cáo tiếp theo: ${timeUntilNextAd}`,
          "custom"
        );
      } else {
        ...
        if (adResult.success) {
          ...
          if (watchResult.success) {
            ...
            const updatedProfileResult = await this.getProfile(
              authorization,
              proxy
            );
            if (updatedProfileResult.success) {
              const nextAdWatchTime = DateTime.fromISO(
                updatedProfileResult.data.adWatchTime,
                { zone: "utc" }
              )
                .setZone("local")
                .plus({ minutes: 2 });
              const timeUntilNextAd = nextAdWatchTime
                .diff(now)
                .toFormat("hh:mm:ss");
              this.log(
                `Thời gian xem quảng cáo tiếp theo: ${timeUntilNextAd}`,
      ...
      this.log(
        `Không thể lấy thông tin tài khoản: ${profileResult.error}`,
        "error"
      );
    }
  }
  ...
  async main() {
    while (true) {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (loginResult.success) {
          ...
          if (profileResult.success) {
            ...
            this.log(
              `Spin ticket: ${profileResult.data.rouletteSpins}`,
              "custom"
            );
            this.log(
              `Circle Balance: ${profileResult.data.starsBalance}`,
              "custom"
            );

            const bonusResult = await this.claimDailyBonus(
              authorization,
              proxy
            );
            if (bonusResult.success) {
              ...
              this.log(
                `Không thể nhận điểm danh hàng ngày: ${bonusResult.error}`,
                "error"
              );
            }
            ...
            if (updatedProfileResult.success) {
              let spinTicket = updatedProfileResult.data.rouletteSpins;
              if (spinTicket <= 0) {
                this.log(colors.yellow("Bạn đã hết lượt spin..."));
              } else {
                while (spinTicket > 0) {
                  const spinResult = await this.spin(authorization, 5, proxy);
                  if (spinResult.success) {
                    this.log(
                      `Spin thành công, nhận ${spinResult.data.prizes.spark} Circles`,
                      "success"
                    );
                  } else {
                    this.log(
                      `Không thể thực hiện spin: ${spinResult.error}`,
                      "error"
                    );
                  }
                  spinTicket -= 5;
              ...
              this.log(
                `Không thể lấy thông tin tài khoản cập nhật: ${updatedProfileResult.error}`,
                "error"
              );
            }
          } else {
      ...
      this.log(
        "Đã xử lý xong tất cả tài khoản. Nghỉ 150 giây trước khi tiếp tục vòng lặp.",
        "warning"
      );
