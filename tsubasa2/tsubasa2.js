                    ...
                    try {
                        ...
                        if (error.response && error.response.status === 400) {
                            if (error.response.data && error.response.data.message === 'Wait for cooldown') {
                                ...
                                this.log(`Lỗi nâng cấp thẻ ${card.name} (${card.cardId}): thẻ đã hết hạn`, 'warning');
                                errorCards.add(card.cardId);
                            } else {
                                this.log(`Lỗi nâng cấp thẻ ${card.name} (${card.cardId}): ${error.response.data.message}`, 'error');
                                errorCards.add(card.cardId);
                            }
                        } else {
