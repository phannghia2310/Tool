...
class PinEye {
  ...
  async managePractice(token) {
    const allPractice = await this.getAllPractice(token);
    ...
    if (allPractice) {
      for (let practice of allPractice) {
        if (practice.nextPracticeTime > 0) {
          this.log(
            colors.yellow(
              `Bạn đã làm ${practice.title} rồi, vui lòng chờ ${practice.nextPracticeTime} giây...`
            )
          );
        } else {
          ...
          if (details) {
            ...
            const claim = await this.claimPractice(token, practice.id);
            if (claim) {
              this.log(
                colors.green(
                  `Claim thành công | Balance: ${claim.balance} | Profit: ${claim.profit}`
                )
              );
            } else {
      ...
      for (let i = 0; i < userData.length; i++) {
        ...
        if (apiResponse && apiResponse.data && apiResponse.data.token) {
          ...
          if (profileResponse && profileResponse.data) {
            ...
            await this.managePractice(token);
          } else {
            this.log(
