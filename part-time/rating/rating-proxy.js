...
class Rating {
  ...
  async authenticate(auth) {
    ...
    try {
      const response = await axios.post(
        url,
        {},
        { headers: this.headers, httpsAgent: this.proxyAgent, timeout: 10000 }
      );
      if (
        response.status === 200 &&
  ...
  async spinRoulette(token) {
    ...
    try {
      const response = await axios.post(
        url,
        {},
        { headers, httpsAgent: this.proxyAgent, timeout: 10000 }
      );
      if (response.status === 200 && response.data.response) {
        return { success: true, data: response.data.response };
      } else {
  ...
  async executeIntegrationTask(token, taskId) {
    ...
    try {
      ...
      if (
        ...
        this.log(
          `Gọi task.integration thành công cho nhiệm vụ ID: ${taskId}`,
          "success"
        );
        return true;
      } else {
        this.log(
          `Gọi task.integration không thành công cho nhiệm vụ ID: ${taskId}: ${JSON.stringify(
            response.data
          )}`,
          "error"
        );
  ...
  async processAppTask(token, task, group) {
    try {
      this.log(
        `Bắt đầu xử lý nhiệm vụ APP: ${task.title} (ID: ${task.id})`,
        "info"
      );
      ...
      if (isSpecialTask) {
        ...
        const dataResponse = await axios.post(
          "https://api.ratingtma.com/task/task.data",
          { task: task.id },
          {
        ...
        if (dataResponse.status === 200) {
          this.log(
            `Gọi task.data thành công cho nhiệm vụ đặc biệt ${task.title}`,
            "success"
          );
        } else {
          this.log(
            `Gọi task.data không thành công cho nhiệm vụ đặc biệt ${
              task.title
        ...
        const appResponse = await axios.post(
          "https://api.ratingtma.com/task/task.app",
        ...
        if (appResponse.status === 200 && appResponse.data.success) {
          this.log(
            `Gọi task.app thành công cho nhiệm vụ ${task.title}`,
            "success"
          );
        } else {
        ...
        if (dataResponse.status === 200 && dataResponse.data.success) {
          this.log(
            `Gọi task.data thành công cho nhiệm vụ ${task.title}`,
            "success"
          );
        } else {
          this.log(
            `Gọi task.data không thành công: ${JSON.stringify(
              dataResponse.data
            )}`,
            "error"
          );
      ...
      if (response.success) {
        const updatedTask = response.data[group].tasks
          .flat()
          .find((t) => t.id === task.id);

        if (updatedTask && updatedTask.order) {
          this.log(
            `Thực hiện executeTaskByOrder cho nhiệm vụ ${task.title} với order: ${updatedTask.order}`,
            "info"
          );
          const executeResult = await this.executeTaskByOrder(
            token,
            group,
            updatedTask.order
          );
          ...
          if (executeResult.success && executeResult.data.result) {
            ...
            this.log(
              `Làm nhiệm vụ APP ${task.title} thành công | phần thưởng ${reward}`,
              "success"
            );
          } else {
            this.log(
              `Không thể hoàn thành nhiệm vụ APP ${task.title}`,
              "error"
            );
          }
        } else {
          this.log(
            `Nhiệm vụ ${task.title} chưa có order hoặc không tìm thấy nhiệm vụ cập nhật`,
            "warning"
          );

          // Xử lý lỗi bằng cách gọi task.integration với task.id 46
          const integrationTaskId = 46;
          const integrationSuccess = await this.executeIntegrationTask(
            token,
            integrationTaskId
          );
          ...
          if (integrationSuccess) {
            ...
            this.log(
              `Đã gọi task.integration cho nhiệm vụ ID: ${integrationTaskId}. Kiểm tra lại order cho nhiệm vụ ${task.title}`,
              "info"
            );
            ...
            if (retryResponse.success) {
              const retriedTask = retryResponse.data[group].tasks
                .flat()
                .find((t) => t.id === task.id);

              if (retriedTask && retriedTask.order) {
                this.log(
                  `Thực hiện executeTaskByOrder cho nhiệm vụ ${task.title} với order: ${retriedTask.order}`,
                  "info"
                );
                const executeResult = await this.executeTaskByOrder(
                  token,
                ...
                if (executeResult.success && executeResult.data.result) {
                  ...
                  this.log(
                    `Không thể hoàn thành nhiệm vụ APP ${
                      task.title
                    } sau khi tích hợp | Lỗi: ${
                      executeResult.error || "Không rõ"
                    }`,
                    "error"
                  );
                }
                ...
                this.log(
                  `Sau khi tích hợp, nhiệm vụ ${task.title} vẫn chưa có order hoặc không tìm thấy nhiệm vụ cập nhật`,
                  "warning"
                );
              }
            } else {
              this.log(
                `Không thể lấy lại danh sách nhiệm vụ sau khi tích hợp: ${retryResponse.error}`,
                "error"
              );
            }
          } else {
            this.log(
              `Gọi task.integration không thành công cho nhiệm vụ ID: ${integrationTaskId}`,
              "error"
            );
          }
        }
        ...
        this.log(
          `Không thể lấy lại danh sách nhiệm vụ sau khi xử lý: ${response.error}`,
          "error"
        );
      }
    } catch (error) {
      this.log(
        `Error processing APP task ${task.id}: ${error.message}`,
        "error"
      );
    }
  }
  ...
  async processLinkTask(token, task, group) {
    try {
      await axios.post(
        "https://api.ratingtma.com/task/task.link",
      ...
      if (response.success) {
        ...
        if (updatedTask && updatedTask.order) {
          ...
          if (executeResult.success && executeResult.data.result) {
            const reward = task.item[0]?.count || "unknown";
            this.log(
              `Làm nhiệm vụ ${task.title} thành công | phần thưởng ${reward}`,
              "success"
            );
          } else {
  ...
  async executeIntegrationTask(token, taskId) {
    try {
      ...
      const integrationResponse = await axios.post(
        "https://api.ratingtma.com/task/task.integration",
        { task: taskId },
        {
          headers: { ...this.headers, Authorization: token },
          httpsAgent: this.proxyAgent,
      ...
      if (
        ...
        integrationResponse.data.response &&
        integrationResponse.data.response.result === true
      ) {
        this.log(
          `Gọi task.integration thành công cho nhiệm vụ ID: ${taskId}`,
          "success"
      ...
      try {
        ...
        if (response.success) {
          ...
          openTasks.forEach((task) =>
            this.log(
              `- ${task.title} (ID: ${task.id}) [Type: ${task.type}]`,
              "custom"
            )
          );
          ...
          this.log(
            `Failed to get tasks for ${group}: ${response.error}`,
            "error"
          );
        }
      } catch (error) {
  ...
  async checkMinigameList(token) {
    const url =
      "https://api.ratingtma.com/game/minigame.list?force_execut=true";
    const headers = {
      ...this.headers,
      Authorization: token,
      "Content-Hello": Math.random().toString(),
    ...
    try {
      ...
      if (response.status === 200 && response.data.response) {
        const comboDay = response.data.response.find(
          (game) => game.key === "combo_day"
        );
  ...
  async collectNyTribute(token) {
    const url = "https://api.ratingtma.com/game/ny.tribute";
    const headers = {
      ...this.headers,
      Authorization: token,
      "Content-Hello": Math.random().toString(),
  ...
  async claimCatchFruit(token) {
    const url = "https://api.ratingtma.com/game/minigame.catchFruit";
    const score = this.getRandomInteger(4000, 5000);
    const headers = {
      ...this.headers,
      Authorization: token,
      "Content-Hello": Math.random().toString(),
  ...
  async boostMinigame(token, minigameId) {
    const url = "https://api.ratingtma.com/game/minigame.boost";
    const headers = {
      ...this.headers,
      Authorization: token,
      "Content-Hello": Math.random().toString(),
  ...
  async manageCatchFruit(token) {
    let getGameInfo = await this.checkCatchFruit(token);
    if (getGameInfo.success) {
      let lock = getGameInfo.hasLock;
      if (lock) {
        this.log(colors.yellow(`Hôm nay bạn đã hết lượt chơi catch fruit...`));
      }
      while (!lock) {
        this.log(colors.cyan(`Bắt đầu chơi catch fruit...`));
        await this.countdown(60);
        const claim = await this.claimCatchFruit(token);
        if (claim.result) {
          this.log(
            colors.green(`Chơi game thành công, nhận được ${claim.score}`)
          );
          this.log(colors.cyan(`Bắt đầu thực hiện boost...`));
          await this.countdown(15);
          const boost = await this.boostMinigame(token, claim.id);
          if (boost) {
            this.log(colors.green(`Boost thành công | Boost: ${boost.boost}`));
          } else {
            this.log(colors.red("Không thể boost game..."));
          }
        } else {
          this.log(colors.red("Lỗi khi claim game catch fruit"));
        }

        getGameInfo = await this.checkCatchFruit(token);
        lock = getGameInfo.hasLock;
      }
    }
  }
  ...
  async promptCombo() {
    return new Promise((resolve) => {
      this.rl.question(
        "Nhập mã combo hôm nay (ví dụ strawberry,orange,watermelon): ",
        (answer) => {
          resolve(answer.trim());
        }
      );
  ...
  async main() {
    ...
    if (data.length !== proxyData.length) {
      this.log(
        `Số lượng proxy (${proxyData.length}) không bằng số lượng tài khoản (${data.length})`,
        "error"
      );
      process.exit(1);
    }
    ...
    while (true) {
      for (let i = 0; i < data.length; i++) {
        ...
        try {
          userId = JSON.parse(
            decodeURIComponent(auth.split("user=")[1].split("&")[0])
          ).id;
        } catch (error) {
          this.log(
            `Lỗi khi phân tích user ID từ dòng ${i + 1}: ${error.message}`,
            "error"
          );
          continue;
        }
        ...
        try {
          ...
          this.log(
            `Proxy cho tài khoản ${i + 1} | ID: ${userId} gặp lỗi: ${
              error.message
            }`,
            "error"
          );
        ...
        const collectNyTribute = await this.collectNyTribute(token);
        if (collectNyTribute) {
          this.log(
            colors.green(
              `Claim thành công profit, nhận ${collectNyTribute.count}`
            )
          );
        }
        ...
        if (comboInput) {
          ...
          if (minigameListResult.success && !minigameListResult.hasLock) {
            ...
            if (comboResult.success) {
              this.log(
                `Nhập combo thành công..score: ${comboResult.score}`,
                "success"
              );
            } else {
        ...
        if (taskListResult.success) {
          const readyTask = taskListResult.data.calendar.tasks[0].find(
            (task) => task.status === "READ"
          );
          if (readyTask) {
            this.log(
              `Tìm thấy nhiệm vụ Daily Rewards Calendar sẵn sàng. Order: ${readyTask.order}`,
              "info"
            );
            const executeResult = await this.executeTaskByOrder(
              token,
            ...
            this.log(
              "Không có nhiệm vụ Daily Rewards Calendar nào sẵn sàng",
              "warning"
            );
          }
        } else {
          this.log(
            `Không thể lấy danh sách nhiệm vụ: ${taskListResult.error}`,
            "error"
          );
        }
        ...
        if (userInfoResult.success) {
          let energy = userInfoResult.data.balances.find(
            (b) => b.key === "energy"
          ).count;
          let ticket = userInfoResult.data.balances.find(
            (b) => b.key === "ticket"
          ).count;
          ...
          while (ticket > 0) {
            ...
            if (spinResult.success) {
              this.log(
                `Spin thành công, nhận được ${spinResult.data.score} score`,
                "success"
              );
              ticket--;
            } else {
          ...
          if (userInfoResult.success) {
            energy = userInfoResult.data.balances.find(
              (b) => b.key === "energy"
            ).count;
            ticket = userInfoResult.data.balances.find(
              (b) => b.key === "ticket"
            ).count;
            this.log(
              `Sau khi spin - Energy: ${energy}, Ticket: ${ticket}`,
              "custom"
            );
          }

          await this.manageCatchFruit(token);
          await this.processAllTaskLists(token);
        } else {
          this.log(
            `Không thể lấy thông tin người dùng: ${userInfoResult.error}`,
            "error"
          );
        }
      ...
      this.log(
        "Đã hoàn thành vòng lặp. Chờ 24 giờ trước khi chạy lại...",
        "info"
      );
